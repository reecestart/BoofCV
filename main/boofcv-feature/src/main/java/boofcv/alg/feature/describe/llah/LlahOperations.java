...
public class LlahOperations {
	...
	private final Map<Integer, FoundDocument> foundMap = new HashMap<>();
	...
	public void lookupDocuments( List<Point2D_F64> dots , double threshold, List<FoundDocument> output ) {
		...
		computeAllFeatures(dots, (dotIdx,l)-> lookupProcessor(foundMap, knownFeatures, featureComputed,l,dotIdx));

		int minimumHits = (int)Math.round(computeMaxUniqueHashPerPoint()*threshold);
		for( var result : foundMap.values() ) {
			int totalValid = 0;
			for (int i = 0; i < result.landmarkHits.size; i++) {
				// Skip if the point was not found
				if( !result.landmarkMask.data[i] )
					continue;

				if( result.landmarkHits.data[i] >= minimumHits ) {
					totalValid++;
				} else {
					// mark it as not found if below the threshold
					result.landmarkMask.data[i] = false;
				}
			}
			if( totalValid > 0 ) {
				output.add(result);
	 ...
	 */
	private void lookupProcessor(Map<Integer, FoundDocument> output, HashSet<LlahFeature> knownFeatures,
								 LlahFeature featureComputed, List<Point2D_F64> pointSet, int dotIdx)
	{
		...
		while( found != null ) {
			...
			FoundDocument results = output.get(found.documentID);
			if( results == null ) {
				results = resultsStorage.grow();
				results.reset();
				results.document = documents.get(found.documentID);
				results.landmarkMask.resize(results.document.landmarks.size);
				results.landmarkMask.fill(false);
				results.landmarkHits.resize(results.document.landmarks.size);
				results.landmarkHits.fill(0);
				results.landmarkToDots.resize(results.document.landmarks.size);
				for (int i = 0; i < results.document.landmarks.size; i++) {
					results.landmarkToDots.get(i).clear();
				}
			...
			results.landmarkMask.set(found.landmarkID,true);
			results.landmarkHits.data[found.landmarkID]++;
	...
	public static class FoundDocument {
		...
		/**
		 * Indicates which landmark indexes were matched. A table is used to make read/write inexpensive
		 */
		public final GrowQueue_B landmarkMask = new GrowQueue_B();
		/**
		...
		public void reset() {
			...
			landmarkMask.reset();
			landmarkHits.reset();
			landmarkToDots.reset();
		}

		public void lookupMatches(FastQueue<PointIndex2D_F64> matches ) {
			matches.reset();
			for (int i = 0; i < landmarkMask.size; i++) {
				if( landmarkMask.get(i) ) {
